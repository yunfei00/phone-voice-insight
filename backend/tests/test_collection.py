...
from apps.collection.tasks import CollectorNotImplementedError, run_collection_task, system_ping
...
@pytest.mark.django_db
def test_unimplemented_collection_task_is_persisted_as_failed(source_target: SourceTarget) -> None:
    task = CollectionTask.objects.create(source_target=source_target)

    with pytest.raises(CollectorNotImplementedError, match="collector not implemented"):
